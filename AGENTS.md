...
- Investigation reads must reflect the latest `origin/master`. If the main workspace is stale (checked out on a non-`master` branch, has dirty files, or is behind the remote), never read project files from it for investigation — use `git show origin/master:<path>` or an up-to-date worktree instead. Reading stale main-workspace files is a bug: it can silently revert newer merged work when the file is later rewritten.
