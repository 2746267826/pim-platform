...
/**
 * ViolinChart.tsx — 小提琴图
 * 密度轮廓+箱体
 * 假数据与 fakeData.ts 同源，支持真实/模拟切换 via useExhibitionData
 */
export interface ViolinChartDatum { label: string; value: number; }
export interface ViolinChartProps {
  data?: ViolinChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: ViolinChartDatum) => void;
  className?: string;
}
const FALLBACK: ViolinChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function ViolinChart({ data, loading, error, height = 168, onSelect, className }: ViolinChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="小提琴图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="小提琴图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
