 ...
 */
export interface TreemapChartDatum { label: string; value: number; }
export interface TreemapChartProps {
  data?: TreemapChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: TreemapChartDatum) => void;
  className?: string;
}
const FALLBACK: TreemapChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function TreemapChart({ data, loading, error, height = 168, onSelect, className }: TreemapChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="树图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="树图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
