 ...
 */
export interface StepChartDatum { label: string; value: number; }
export interface StepChartProps {
  data?: StepChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: StepChartDatum) => void;
  className?: string;
}
const FALLBACK: StepChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function StepChart({ data, loading, error, height = 168, onSelect, className }: StepChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="步进图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="步进图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
