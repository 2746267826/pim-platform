...
/**
 * HorizontalBarChart.tsx — 水平柱状图
 * 标签易读，适合排行
 * 假数据与 fakeData.ts 同源，支持真实/模拟切换 via useExhibitionData
 */
export interface HorizontalBarChartDatum { label: string; value: number; }
export interface HorizontalBarChartProps {
  data?: HorizontalBarChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: HorizontalBarChartDatum) => void;
  className?: string;
}
const FALLBACK: HorizontalBarChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function HorizontalBarChart({ data, loading, error, height = 168, onSelect, className }: HorizontalBarChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="水平柱状图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="水平柱状图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
