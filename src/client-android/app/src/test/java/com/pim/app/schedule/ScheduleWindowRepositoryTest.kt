...
class ScheduleWindowRepositoryTest {
    ...
    @Test
    fun upcomingReturnsOnlyFutureWindowsWithLocation() {
        val windows = listOf(
        ...
        assertEquals(listOf("future"), ScheduleWindowSelector.upcoming(windows, nowMillis = 10_000L).map { it.id })
    }

    @Test
    fun mapsApiEventsWithLocationsToScheduleWindows() {
        ...
        assertEquals("上海市黄浦区", windows.single().locationText)
    }
}
