    ...
    IAiGateway aiGateway)
{
        ...
        CancellationToken ct = default)
    {
        ...
        var result = await aiGateway.CompleteAsync(request, ct);
        if (!IsSuccessful(result))
            return null;

        using var outputDocument = ParseOutput(result);
    ...
    public static void RegisterSchemas(IAiSchemaRegistry registry)
    {
        registry.Register(new AiSchemaDefinition(
            ...
            """,
            Description: "Summarizes file evidence with tags, language, and sensitivity."));
        registry.Register(new AiSchemaDefinition(
            SuggestionsSchemaName,
            SchemaVersion,
            """
            ...
            """,
            Description: "Suggests non-executing file organization improvements."));
    }
    ...
    private AiGatewayRequest BuildGatewayRequest(
        ...
        => new(
            ...
            [
                new AiMessage(AiMessageRole.System, "You analyze indexed file evidence and return only JSON matching the requested schema."),
    ...
    private static bool IsSuccessful(AiResult result)
        => result.Status is AiRequestStatus.Succeeded
            && (!string.IsNullOrWhiteSpace(result.ParsedOutputJson)
                || !string.IsNullOrWhiteSpace(result.ResponseText));

    private static JsonDocument ParseOutput(AiResult result)
        => JsonDocument.Parse(
            !string.IsNullOrWhiteSpace(result.ParsedOutputJson)
                ? result.ParsedOutputJson
                : result.ResponseText ?? "{}");
