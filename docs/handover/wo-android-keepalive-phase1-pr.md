...
> 本文件是阶段一 PR 的描述草稿，用于让 PR 正文保持可复核。**证据绑定 head `a4690ac5f9b3614b37fd2189141c5a2a5969f07b`**。
...
所有命令均在本机执行，结果取自 head `a4690ac5f9b3614b37fd2189141c5a2a5969f07b`。
...
排除说明（沿用 CI `build-api.yml` 的既有排除项，外加一条）：

- 前 4 条是 CI 也排除的"需要生产镜像库"用例；
- `DeviceMergeRealDbTests` 的 `MergeAsync_OnProductionShapedData_ResolvesCrossDeviceDuplicateKeys` 在**基线 `23cf6d27` 上同样失败**（我在独立 worktree 上复跑确认），前提是"镜像库里同一个用户至少有两台设备且存在跨设备重复业务键"，本机隔离库不满足该前提。属于**既有问题，与本次改动无关**。
...
证据均绑定 head `a4690ac5f9b3614b37fd2189141c5a2a5969f07b`。结果状态只用 `PASS` / `FAIL` / `BLOCKED` / `NOT-VERIFIED` / `APPROVED-EXCEPTION`。
...
| AC-31.3 | 本 PR 含逐条 AC 映射表，证据绑定 head SHA | 本 PR 正文 | PASS | 本表，head `a4690ac5` |
