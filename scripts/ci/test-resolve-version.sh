...
assert_eq version "2026.07.42" $WORKDIR/rv-master.env
...
assert_eq year_month "2026.07" $WORKDIR/rv-master.env
assert_eq artifact_slug "2026.07.42" $WORKDIR/rv-master.env
...
run_case pr \
  env GITHUB_REF=refs/pull/12/merge GITHUB_RUN_NUMBER=42 GITHUB_SHA=abcdef1234567890 \
      ...
      PR_NUMBER=12 \
      bash "$SCRIPT" --date 2026-07-12 --print-env
assert_eq version "2026.07.42-pr.12+abcdef1" $WORKDIR/rv-pr.env
assert_eq version_code "100042" $WORKDIR/rv-pr.env
assert_eq is_release "false" $WORKDIR/rv-pr.env
assert_eq artifact_slug "2026.07.42-pr.12-abcdef1" $WORKDIR/rv-pr.env
...
run_case dev \
  env GITHUB_REF=refs/heads/codex/foo GITHUB_RUN_NUMBER=7 GITHUB_SHA=deadbeefcafebabe \
      ...
      bash "$SCRIPT" --date 2026-07-12 --print-env
assert_eq version "2026.07.7-dev+deadbee" $WORKDIR/rv-dev.env
...
run_case patch \
  env GITHUB_REF=refs/heads/master GITHUB_RUN_NUMBER=42 GITHUB_SHA=abcdef1234567890 \
      ...
      bash "$SCRIPT" --date 2026-07-12 --print-env
assert_eq version "2026.07.42+android.1" $WORKDIR/rv-patch.env
