...
/**
 * VerticalBarChart.tsx — 垂直柱状图
 * 最基础的柱状，适合 TopN 对比
 * 假数据与 fakeData.ts 同源，支持真实/模拟切换 via useExhibitionData
 */
export interface VerticalBarChartDatum { label: string; value: number; }
export interface VerticalBarChartProps {
  data?: VerticalBarChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: VerticalBarChartDatum) => void;
  className?: string;
}
const FALLBACK: VerticalBarChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function VerticalBarChart({ data, loading, error, height = 168, onSelect, className }: VerticalBarChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="垂直柱状图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="垂直柱状图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
