...
/**
 * ParallelChart.tsx — 平行坐标图
 * 多指标平行轴
 * 假数据与 fakeData.ts 同源，支持真实/模拟切换 via useExhibitionData
 */
export interface ParallelChartDatum { label: string; value: number; }
export interface ParallelChartProps {
  data?: ParallelChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: ParallelChartDatum) => void;
  className?: string;
}
const FALLBACK: ParallelChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function ParallelChart({ data, loading, error, height = 168, onSelect, className }: ParallelChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="平行坐标图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="平行坐标图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
