...
public class FileAiServiceTests
{
    ...
    public async Task GenerateSummaryAndTagsAsync_SendsGovernedGatewayRequestAndStoresResult()
    {
        ...
        Assert.Equal(item.Id, request.SourceObjectId);
    ...
    private static AiGatewayResult SuccessfulResult(Guid logId, string json, string model)
        => new(
            "succeeded",
            json,
            JsonDocument.Parse(json),
            [],
            new AiTokenUsage(10, 20, 30),
            logId,
            null,
            model);
    ...
    private sealed class FakeAiGateway : IAiGateway
    {
        ...
        public AiGatewayResult Result { get; set; } = new(
            "failed",
            null,
            null,
            [],
