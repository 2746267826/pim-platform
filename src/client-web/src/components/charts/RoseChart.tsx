 ...
 */
export interface RoseChartDatum { label: string; value: number; }
export interface RoseChartProps {
  data?: RoseChartDatum[];
  loading?: boolean;
  error?: string | null;
  height?: number;
  onSelect?: (item: RoseChartDatum) => void;
  className?: string;
}
const FALLBACK: RoseChartDatum[] = [
  { label: "A", value: 32 },
  { label: "B", value: 28 },
  { label: "C", value: 24 },
  { label: "D", value: 18 },
...
export default function RoseChart({ data, loading, error, height = 168, onSelect, className }: RoseChartProps) {
  ...
  return (
    <div className={className} role="img" aria-label="南丁格尔玫瑰图" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' && onSelect && src[0]) onSelect(src[0]); }}>
      <EChartBox option={option} height={height} ariaLabel="南丁格尔玫瑰图" onEvents={onSelect ? { click: (p: unknown) => { const d = p as { name?: string }; const it = src.find(x => x.label === d.name); if (it) onSelect(it); } } : undefined} />
    </div>
  );
}
